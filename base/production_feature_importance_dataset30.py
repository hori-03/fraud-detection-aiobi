...
"""Script de production pour extraire l'importance des features du modèle XGBoost Dataset30
Version adaptée pour Dataset30 (42K lignes avec fraud 3.38% - Loan Default Fraud)
"""
...
def load_model_and_data():
    ...
    # Chemins - Dataset30
    model_path = Path("data/models/Dataset30_xgb_model.joblib")
    grid_results_path = Path("data/results/Dataset30_grid_search_results.json")
    data_path = Path("data/datasets/Dataset30.csv")
...
def prepare_features_for_analysis(df):
    """Préparer les features comme dans l'entraînement (Dataset30 - 42K lignes, fraud 3.38%, Loan Default Fraud)"""
    print("\n=== PRÉPARATION DES FEATURES ===")
    
    # Feature engineering identique à baseline_xgboost.py Dataset30
    df = df.copy()
    
    # Exclure les colonnes non-features (Dataset30 - target: default_fraud_flag, IDs: loan_transaction_id, borrower_id)
    columns_to_exclude = [
    ...
    # Dataset30 A des features credit risk (LOAN DEFAULT FRAUD - debt_to_income_ratio, delinquencies, income)
    if 'debt_to_income_ratio' in df.columns:
        ...
        print("Features debt_to_income créées")
    ...
    if 'number_of_delinquencies' in df.columns:
        ...
        print("Features delinquency créées")
    ...
    if 'annual_income_k_usd' in df.columns:
        ...
        print("Features income créées")
    
    # Encodage des variables catégorielles restantes (Dataset30 - LOAN DEFAULT FRAUD)
    print("Encodage des variables catégorielles...")
    # Dataset30 utilise 'default_fraud_flag' comme target, exclure aussi: loan_transaction_id, borrower_id, tx_timestamp
    categorical_cols = [col for col in df.select_dtypes(include=['object']).columns 
                        if col not in ['default_fraud_flag', 'loan_transaction_id', 'borrower_id', 'tx_timestamp', 'timestamp']]
    ...
    # Étape 1: amount_log, amount_squared, amount_sqrt (Dataset30 utilise 'loan_amount_fcfa')
    if 'loan_amount_fcfa' in df.columns:
    ...
    # Étape 2: Binning amount (Dataset30 utilise 'loan_amount_fcfa')
    if 'loan_amount_fcfa' in df.columns:
        if 'amount_zscore' not in df.columns:
            df['amount_zscore'] = (df['loan_amount_fcfa'] - df['loan_amount_fcfa'].mean()) / df['loan_amount_fcfa'].std()
        ...
        print("Features loan_amount_fcfa créées")
    elif 'transaction_amount_fcfa' in df.columns:
...
def main():
    """Fonction principale"""
    print("=" * 80)
    print(" EXTRACTION IMPORTANCE DES FEATURES - Dataset30 (42K lignes, 3.38% fraud) ")
    print("=" * 80)
    ...
    try:
        ...
        output_path = save_feature_importance(feature_importance_df, best_params, best_score, "Dataset30")
