...
"""Script de production pour extraire l'importance des features du modèle XGBoost Dataset29
Version adaptée pour Dataset29 (70K lignes avec fraud 1.17% - Chargeback Fraud)
"""
...
def load_model_and_data():
    ...
    # Chemins - Dataset29
    model_path = Path("data/models/Dataset29_xgb_model.joblib")
    grid_results_path = Path("data/results/Dataset29_grid_search_results.json")
    data_path = Path("data/datasets/Dataset29.csv")
...
def prepare_features_for_analysis(df):
    """Préparer les features comme dans l'entraînement (Dataset29 - 70K lignes, fraud 1.17%, Chargeback Fraud)"""
    print("\n=== PRÉPARATION DES FEATURES ===")
    
    # Feature engineering identique à baseline_xgboost.py Dataset29
    df = df.copy()
    
    # Exclure les colonnes non-features (Dataset29 - target: chargeback_fraud, IDs: pos_transaction_id, merchant_id, terminal_id)
    columns_to_exclude = [
    ...
    # Features temporelles depuis tx_timestamp (Dataset29 a tx_timestamp)
    ...
    # Dataset29 A des features merchant (CHARGEBACK FRAUD - merchant_risk_score, cashback_amount)
    if 'merchant_risk_score' in df.columns:
        df['merchant_risk_log'] = np.log1p(df['merchant_risk_score'])
        df['is_high_risk_merchant'] = (df['merchant_risk_score'] > df['merchant_risk_score'].quantile(0.75)).astype(int)
        df['is_low_risk_merchant'] = (df['merchant_risk_score'] < df['merchant_risk_score'].quantile(0.25)).astype(int)
        print("Features merchant_risk créées")
    
    # Features cashback (Dataset29 A cashback_amount + transaction_amount_fcfa)
    if 'cashback_amount' in df.columns:
        df['cashback_log'] = np.log1p(df['cashback_amount'])
        df['has_cashback'] = (df['cashback_amount'] > 0).astype(int)
        if 'transaction_amount_fcfa' in df.columns:
            df['cashback_to_amount_ratio'] = df['cashback_amount'] / (df['transaction_amount_fcfa'] + 1)
        print("Features cashback créées")
    
    # Encodage des variables catégorielles restantes (Dataset29 - CHARGEBACK FRAUD)
    print("Encodage des variables catégorielles...")
    # Dataset29 utilise 'chargeback_fraud' comme target, exclure aussi: pos_transaction_id, merchant_id, terminal_id, tx_timestamp
    categorical_cols = [col for col in df.select_dtypes(include=['object']).columns 
                        if col not in ['chargeback_fraud', 'pos_transaction_id', 'merchant_id', 'terminal_id', 'tx_timestamp', 'timestamp']]
    ...
    # Étape 1: amount_log, amount_squared, amount_sqrt (Dataset29 utilise 'transaction_amount_fcfa')
    if 'transaction_amount_fcfa' in df.columns:
    ...
    elif 'claim_amount_fcfa' in df.columns:
    ...
    # Étape 2: Binning amount (Dataset29 utilise 'transaction_amount_fcfa')
    if 'transaction_amount_fcfa' in df.columns:
        df['amount_zscore'] = (df['transaction_amount_fcfa'] - df['transaction_amount_fcfa'].mean()) / df['transaction_amount_fcfa'].std()
...
def main():
    """Fonction principale"""
    print("=" * 80)
    print(" EXTRACTION IMPORTANCE DES FEATURES - Dataset29 (70K lignes, 1.17% fraud) ")
    print("=" * 80)
    ...
    try:
        ...
        output_path = save_feature_importance(feature_importance_df, best_params, best_score, "Dataset29")
